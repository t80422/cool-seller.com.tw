...
var containerEl = document.querySelector('.portfolio-wrap');
...
function getGroupsState() {
    ...
    var groupsState = {
        industry: mixer.getFilterGroupSelectors('industry').map(getValueFromSelector),
...
function filterMixerByGroupsState(groupsState, animate) {
    var industry = (groupsState && groupsState.industry) ? groupsState.industry : [];
    var type = (groupsState && groupsState.type) ? groupsState.type : [];

    mixer.setFilterGroupSelectors('industry', industry.map(getSelectorFromValue));
